...
puerto = 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3dlYnBsYXkxMC9XRUJQTEFZL21hc3Rlci9DYXRlZ29yaWFz=='.decode('base64')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[plugin.video.WEBPLAY-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Liquid,Ops algo de errado aconteceu - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Liquid,Erro servidor está offline. - "+str(e.reason)+",10000,"+icon+")")
...
def menu_donate():
    ...
    if link == 0:
        link2 = dialog.select('Doação Via Mercado Pago', ['5 Reais','10 Reais','20 Reais','25 Reais','30 Reais','35 Reais','40 Reais','45 Reais','50 Reais'])

        if link2 == 0:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-73f9e3a5-760f-4aba-82b1-5d923a1df234"
            web_browser(url)
        if link2 == 1:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-e15f8cf9-9960-4364-9d61-ce5383e443b6"
            web_browser(url)
        if link2 == 2:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-efe8999e-a072-47eb-9df9-2570ec4ea6e1"
            web_browser(url)
        if link2 == 3:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-07e2152f-fc49-48dc-a95b-59bbe798f792"
            web_browser(url)
        if link2 == 4:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-a84fc739-f7dc-494d-b337-b3659cff7e03"
            web_browser(url)
        if link2 == 5:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-0d9000fd-fdef-4801-ae76-c4337b57b2dc"
            web_browser(url)
        if link2 == 6:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-b460093c-70ec-4721-b088-d4c7b19fe2b2"
            web_browser(url)
        if link2 == 7:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-432c5d6e-23c2-469b-83fe-3f8a6d2e4634"
            web_browser(url)
        if link2 == 8:
            url = "https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=470412597-eaf44ee8-77e8-4a60-a714-48a495f0225e"
            web_browser(url)
        #if link == 9:
            
            
    if link == 1:
        url = "https://pag.ae/7UNxrTtr1"
        web_browser(url)
...
def SKindex():
    ...
    msg = ("https://raw.githubusercontent.com/webplay10/WEBPLAY/master/Alarme") 
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[firehd Lists-%s]: %s" %(addon_version, string))	
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                addon_log('We failed with error code - %s.' % e.code)
                xbmc.executebuiltin("XBMC.Notification(firehd,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(firehd,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
	...
	try:
		if os.path.exists(favorites) == True:
			...
			if FAV == "[]":
				...
				addDir('[COLOR blue][B]firehd Favoritos[/B][/COLOR]','url',4,os.path.join(home, 'fanart.jgp'),FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(firehd,New source added.,5000,"+icon+")")
        if not url is None:
...
def download_file(name, url):
        ...
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('firehdTV', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from firehd Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                ...
                contextMenu.append(('Adicionar [COLOR blue][B]firehd[/B][/COLOR] [COLOR red][B]TV[/B][/COLOR]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from firehd Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                if regexs:
                    fav_params += "&regexs="+regexs
                contextMenu.append(('Add to firehd Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(firehd,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(firehd,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(firehd,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(firehd,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(firehd,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
